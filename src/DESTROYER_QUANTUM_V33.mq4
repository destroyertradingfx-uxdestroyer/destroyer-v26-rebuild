...
#property version   "33.00"  // V33.0 DESTROYER: Warden+Titan filter cleanup from V28 base
#property strict
...
extern bool    InpMeanReversion_Enabled= false;       // V28: DISABLED -- PF 1.05 = breakeven, dead weight
...
void ExecuteTitanStrategy()
{
    ...
    // V33: SIMPLIFIED TREND -- just EMA, no Kalman, no volatility filters
    ...
    bool d1Uptrend = (d1_ema_fast > d1_ema_slow) && (d1_price > d1_ema_fast);
    bool d1Downtrend = (d1_ema_fast < d1_ema_slow) && (d1_price < d1_ema_fast);
    
    if(!d1Uptrend && !d1Downtrend) return;
    ...
    bool h4Aligned = (d1Uptrend && h4_price > h4_ema_fast) || 
                     (d1Downtrend && h4_price < h4_ema_fast);
    if(!h4Aligned) return;
    
    // V33: Entry -- pullback to H4 50 EMA + strong candle
    if(d1Uptrend)
    {
        bool pullback = (Low[2] <= h4_ema_slow + (5 * Point));
        double bodySize = MathAbs(Close[1] - Open[1]);
        double totalRange = High[1] - Low[1];
        
        if(pullback && bodySize > (totalRange * 0.5))
        {
            double adaptiveSL = Low[1] - (currentATR * 1.2);
            double minSLDistance = (Ask - Bid) * 3;
            if((Ask - adaptiveSL) < minSLDistance) adaptiveSL = Ask - minSLDistance;
            
            double lots = MoneyManagement_Quantum(InpTitan_MagicNumber, InpBase_Risk_Percent);
            if(lots > 0) OpenTrade(OP_BUY, lots, Ask, adaptiveSL, 0, "TITAN_V33_BUY", InpTitan_MagicNumber);
        }
    }
    else if(d1Downtrend)
    {
        bool pullback = (High[2] >= h4_ema_slow - (5 * Point));
        double bodySize = MathAbs(Close[1] - Open[1]);
        double totalRange = High[1] - Low[1];
        
        if(pullback && bodySize > (totalRange * 0.5))
        {
            double adaptiveSL = High[1] + (currentATR * 1.2);
            double minSLDistance = (Ask - Bid) * 3;
            if((adaptiveSL - Bid) < minSLDistance) adaptiveSL = Bid + minSLDistance;
            
            double lots = MoneyManagement_Quantum(InpTitan_MagicNumber, InpBase_Risk_Percent);
            if(lots > 0) OpenTrade(OP_SELL, lots, Bid, adaptiveSL, 0, "TITAN_V33_SELL", InpTitan_MagicNumber);
        }
    }
}
...
void ExecuteWardenStrategy()
{
    ...
    // V33: REMOVED IsReaperConditionMet() filter
    // Was contradictory: mean reversion filter on a breakout strategy
    // Warden breaks out of BB squeeze — doesn't need RSI at extremes
    ...
    if(isSqueezeOn)
    {
        ...
        // Breakout Confirmation: bar range > ATR(10)
        ...
        // V33: RELAXED volume filter -- was Volume[1] > Volume[2], now uses 3-bar avg
        double avgVol3 = (Volume[1] + Volume[2] + Volume[3]) / 3.0;
        bool volumeOK = (Volume[1] > avgVol3 * 0.8); // 80% of 3-bar average
        
        // Buy Breakout: Close above BB upper + momentum confirmation + range confirmation + volume
        // V33: Relaxed ATR beyond band from 20% to 10%
        if(Close[1] > bb_upper_break && (Close[1] - bb_upper_break) > (iATR(Symbol(), Period(), 14, 1) * 0.1) && Close[1] > momentum_ma && breakout_confirmed && volumeOK)
        {
            ...
            if(Global_Risk_Check(lots, slPoints))
            {
               if(lots > 0)
               {
                  OpenTrade(OP_BUY, lots, Ask, sl, tp, "WARDEN_BUY_V33", InpWarden_MagicNumber);
               }
            }
        }
        // Sell Breakout: Close below BB lower + momentum confirmation + range confirmation + volume
        // V33: Relaxed ATR beyond band from 20% to 10%
        else if(Close[1] < bb_lower_break && (bb_lower_break - Close[1]) > (iATR(Symbol(), Period(), 14, 1) * 0.1) && Close[1] < momentum_ma && breakout_confirmed && volumeOK)
        {
            double slPoints = CalculateStopLoss_Warden();
            ...
            if(Global_Risk_Check(lots, slPoints))
            {
               if(lots > 0)
               {
                  OpenTrade(OP_SELL, lots, Bid, sl, tp, "WARDEN_SELL_V33", InpWarden_MagicNumber);
               }
            }
        }
    }
