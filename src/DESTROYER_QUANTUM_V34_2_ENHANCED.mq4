...
#property version   "28.00"  // V28.0 DESTROYER REBUILD: Undertrading fix + filter relaxation + strategy rebalance
#property strict
...
void ExecuteTitanStrategy()
{
   ...
   // Max 3 sequential filters per strategy architecture rule.
    if(Period() != PERIOD_H4) return;
    if(!InpTitan_Enabled) return; // LEVIATHAN: All strategies enabled // V34: Restored enabled check (was LEVIATHAN override)
...
void ExecuteWardenStrategy()
{
   ...
   if(!IsDrawdownSafe()) {
    ...
    // V34 FIX: REMOVED IsReaperConditionMet() from Warden. Breakout strategy != mean-reversion.
    {
   // V34 FIX: Removed old IsReaperConditionMet filter log
   // V34 FIX: Removed old IsReaperConditionMet return
    }
    ...
    if(isSqueezeOn)
    {
        ...
        if(Close[1] > bb_upper_break && (Close[1] - bb_upper_break) > (iATR(Symbol(), Period(), 14, 1) * 0.2) && Close[1] > momentum_ma && breakout_confirmed && iATR(Symbol(), Period(), 14, 1) > iATR(Symbol(), Period(), 14, 2) * 0.8)
        {
        ...
        // PHOENIX UPGRADE: Ensure the close is at least 20% of an ATR value beyond the band
        else if(Close[1] < bb_lower_break && (bb_lower_break - Close[1]) > (iATR(Symbol(), Period(), 14, 1) * 0.2) && Close[1] < momentum_ma && breakout_confirmed && iATR(Symbol(), Period(), 14, 1) > iATR(Symbol(), Period(), 14, 2) * 0.8)
        {
            // PHASE 3: Use Warden's Hard Deck Stop Loss
            double slPoints = CalculateStopLoss_Warden();
...
public:
   ...
   // V34 FIX: Disabled BoostMeanReversionPerformance() call - force-enable bug. BoostMeanReversionPerformance();
   ...
   // V34 FIX: REMOVED force-enable - InpMeanReversion_Enabled = true; // WAS A BUG
